...
fn tiles(w: u32, h: u32) -> dom::TileSize {
    dom::TileSize { w, h }
}

fn plain(tag: dom::Tag) -> dom::Element {
    dom::Element {
        key: None,
        tag,
        attrs: vec![],
...
/// Builds the flat node table: `element`/`text`/`slot` append a node and return
...
impl Builder {
    ...
    fn element(&mut self, parent: Option<u32>, el: dom::Element) -> u32 {
        self.push(parent, dom::NodeKind::Element(el))
    }
    fn text(&mut self, parent: u32, value: &str) -> u32 {
        self.push(Some(parent), dom::NodeKind::Text(value.to_string()))
    }
    fn slot(&mut self, parent: u32, name: &str) -> u32 {
